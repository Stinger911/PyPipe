...
class AsyncFunctionalTransformation(AsyncTransformation):
    ...
    def __init__(self, process_func: Callable[[AsyncIterable[Any]], AsyncIterable[Any]]):
        self._process_func = process_func

    def process(self, data: AsyncIterable[Any]) -> AsyncIterable[Any]:
        return self._process_func(data)

def async_transformation(func: Callable[[AsyncIterable[Any]], AsyncIterable[Any]]) -> AsyncFunctionalTransformation:
...
class AsyncPipeline:
    ...
    def add(self, transformation: AsyncTransformation, index: int | None = None) -> "AsyncPipeline":
        if index is None:
